...
export const sendOTP = async (phone: string): Promise<{ error?: AuthError }> => {
  try {
...
export const signInWithPhone = async (phone: string, password: string): Promise<AuthResponse> => {
  try {
    const { data, error } = await supabase.auth.signInWithPassword({
      phone,
      password,
...
export const signInWithProvider = async (provider: 'google' | 'facebook' | 'apple'): Promise<AuthResponse> => {
  try {
    ...
    // OAuth returns URL for web authentication
    return { error: { message: 'OAuth authentication initiated. Please complete in browser.' } };
  } catch (error) {
    return { error: { message: error instanceof Error ? error.message : 'Unknown error' } };
  }
};
